...
export function ThinkingIndicator({ text, estimatedTime, elapsedSeconds = 0 }: ThinkingIndicatorProps) {
  ...
  return (
    <div className="ag-glass flex flex-col sm:flex-row sm:items-center space-y-2 sm:space-y-0 sm:space-x-3 rounded-2xl px-4 py-2.5 my-2 w-max max-w-[650px] shadow-lg">
      <div className="flex items-center space-x-2.5 min-w-0">
        ...
        <div className="flex space-x-0.5 shrink-0 ml-1">
          ...
          <div className="w-1 h-1 bg-fuchsia-400 rounded-full animate-bounce" style={{ animationDelay: '300ms' }} />
        </div>
      </div>
      ...
      <div className="flex items-center space-x-2 shrink-0 sm:border-l sm:border-zinc-800/80 sm:pl-3">
        ...
        <div className="flex items-center space-x-1 px-2 py-0.5 bg-indigo-500/10 border border-indigo-500/25 rounded-md text-[11px] font-mono text-indigo-200">
          <Clock size={11} className="text-indigo-300 animate-spin" style={{ animationDuration: '4s' }} />
          <span>{formatSeconds(elapsedSeconds)}</span>
        </div>

        {/* Estimated Time Badge */}
        {estimatedTime && (
          <div className="flex items-center space-x-1 px-2 py-0.5 bg-white/[0.04] border border-white/10 rounded-md text-[11px] font-mono text-zinc-400">
